...
define( 'THEME_VERSION', '6.12.3' ); // 主题版本号，请勿修改！！！
    ...
    function wpcom_style_output($css){
        ...
        if(isset($options['dark_style']) && $options['dark_style'] == '2'){ ?>
            @media (prefers-color-scheme: dark) {
                ...
                <?php
                $logo_dark = isset($options['dark_style_logo']) && $options['dark_style_logo'] ? wp_get_attachment_url($options['dark_style_logo']) : '';
                if($logo_dark){ ?>
                .header .logo a, .member-form-head .member-form-logo{display: block;background-image: url('<?php echo $logo_dark;?>');background-size: auto 100%;background-repeat: no-repeat;}
        ...
        <?php }else if(isset($options['dark_style_toggle']) && $options['dark_style_toggle'] == '1'){
            $logo_dark = isset($options['dark_style_logo']) && $options['dark_style_logo'] ? wp_get_attachment_url($options['dark_style_logo']) : '';?>
            <?php if($logo_dark){ ?>
                .style-for-dark .header .logo a, .style-for-dark .member-form-head .member-form-logo{display: block;background-image: url('<?php echo $logo_dark;?>');background-size: auto 100%;background-repeat: no-repeat;}
        ...
        if(isset($sizes['post-thumbnail']) && $sizes['post-thumbnail']) {
            ...
            .post-loop-default .item-img:before,.wpcom-profile-main .profile-posts-list .item-img:before{padding-top: <?php echo $bl;?>%!important;}
...
function wpcom_video_height($scripts){
    global $options;
    $scripts['video_height'] = intval(isset($options['post_video_height']) && $options['post_video_height'] ? $options['post_video_height'] : 482);
    ...
    if($lang == 'zh_CN'){
        $font_url = 'https://googlefonts.wp-china-yes.net/css2' . $font_str;
    }else{
    ...
    if((isset($options['google-font-local']) && $options['google-font-local'] == '1') || !isset($options['google-font-local'])){
        $font_url = WPCOM_Static_Cache::get_font_css($font_url);
    }
