<div class="hidden-content-wrap" id="<?php echo esc_attr($id);?>">
    ...
    <div class="hidden-content">
        ...
        if($type == '0'){ ?>
            ...
            <a class="btn btn-primary hidden-content-btn hidden-content-btn-comment" href="#comments">立即回复</a>
            <div class="hidden-content-refresh">已经回复？<a class="refresh-url j-refresh-hidden-content" href="javascript:;">立即刷新<?php WPCOM::icon('refresh')?></a></div>
        <?php }else if($type == '1'){ ?>
            ...
            <a class="btn btn-primary hidden-content-btn hidden-content-btn-login" href="<?php echo wp_login_url();?>">立即登录</a>
            <div class="hidden-content-refresh">已经登录？<a class="refresh-url j-refresh-hidden-content" href="javascript:;">立即刷新<?php WPCOM::icon('refresh')?></a></div>
        <?php } else if($type == '2'){
            global $current_user;
            ...
            <?php }else{ ?>
                <a class="btn btn-primary hidden-content-btn hidden-content-btn-login" href="<?php echo wp_login_url();?>">立即登录</a>
            <?php } ?>
            <div class="hidden-content-refresh">已有权限？<a class="refresh-url j-refresh-hidden-content" href="javascript:;">立即刷新<?php WPCOM::icon('refresh')?></a></div>
        <?php }else if($type == '3'){ ?>
            ...
            <div class="hidden-content-input">
                ...
                <button class="btn btn-primary hidden-content-btn hidden-content-btn-password" type="button">立即解锁</button>
            </div>
        <?php } ?>
    </div>
</div>