...
jQuery(function($) {
    ...
    let $toggle = $('.dark-style-toggle');
    if (($toggle.length === 0 && _wpcom_js.dark_style && _wpcom_js.dark_style == '1') || (_wpcom_js.dark_style && _wpcom_js.dark_style == '2' && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
        setEditorStyle(1, 1);
    } else if ($toggle.length) {
        let is_dark = $toggle.hasClass('active');
        setEditorStyle(is_dark, 1);
    }

    function setEditorStyle(dark, load) {
        // 夜间模式，设置编辑器颜色
        setTimeout(() => {
            if (typeof tinymce !== 'undefined' && tinymce.editors) {
                tinymce.editors.map(editor => {
                    editor.contentWindow.document.body.style.color = dark ? '#fff' : '#232339';
                    dark ? editor.contentWindow.document.body.classList.add('style-for-dark') : editor.contentWindow.document.body.classList.remove('style-for-dark');
                });
            }
            if (load) {
                setTimeout(() => {
    ...
    function mobile_dropdown_menu(){
        if(!$navbar){
            ...
            for(var i=0;i<$dropdown.length;i++){
                var $item =  $($dropdown[i]);
                if($item.find('.m-dropdown').length==0) $item.append('<div class="m-dropdown"><i class="wpcom-icon wi"><svg aria-hidden="true"><use xlink:href="#wi-arrow-down-3"></use></svg></i></div>');
            }
        }
    }
        ...
        $wrap.find('.post-loop-masonry').each(function (i, el) {
            ...
            $(el).find('.item-thumb img').one('load', function () {
                if (loaded === 1) { // 需要有一张图片加载完成以后再渲染，通常第一张图片是 lazyload替代图
                    init_macy(el);
                } else if (loaded === 2) { // 触发重新渲染，第二张图片才能确定是文章图片，主要针对iOS优化
    ...
    $('.navbar-search').on('keydown', '.navbar-search-input', function(){
        ...
        if($.trim($el.find('.navbar-search-input').val())==''){
            $el.addClass('warning');
            $el.find('.navbar-search-input').trigger('focus');
            return false;
        }
    });
    ...
    $(document).on('click', function (e) {
        ...
        let $el = $(this);
        let is_dark = $el.hasClass('active');
        $el.addClass('loading');
        window.localStorage && localStorage.setItem('darkStyle', is_dark ? 0 : 1);
        setEditorStyle(is_dark ? 0 : 1);
        if (is_dark) {
            $('body').removeClass('style-for-dark');
            setTimeout(() => {
                $el.find('use')[0].setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href', '#wi-sun-fill');
            ...
            setTimeout(() => {
                $el.find('use')[0].setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href', '#wi-moon-fill');
                $el.addClass('active');
                $el.removeClass('loading');
            }, 580);
        }
    ...
    $('#commentform').on('submit', function(){
        ...
        if($.trim($content.val())==''){
            $content.addClass('error').trigger('focus');
            focus = 1;
            err = 1;
        }
    ...
    const $readmore = $('.entry-readmore');
    if($readmore.length){
        const $content = $('.entry-content');
        let outerHeight = $content.outerHeight();
        if ($content.find('.entry-copyright').length) {
            outerHeight -= $content.find('.entry-copyright').outerHeight();
        }
        if(outerHeight > $win.height() + 150) {
            let pct = (outerHeight - $win.height()) / outerHeight;
            let text = _wpcom_js.js_lang.expand_more.replace('%s', parseInt(pct * 100) + '%');
            $readmore.find('.entry-readmore-btn').html(text +'<i class="wpcom-icon wi"><svg aria-hidden="true"><use xlink:href="#wi-expand"></use></svg></i>');
            $content.css({
                height: $win.height(),
                overflow: 'hidden'
            });
            $readmore.on('click', '.entry-readmore-btn', function () {
    ...
    $('#wrap').on('click', '.j-newslist .tab', function () {
        ...
        $content.eq($this.index()).append('<i class="wpcom-icon wi wi-loader"><svg aria-hidden="true"><use xlink:href="#wi-loader"></use></svg></i>');
        ...
        $.ajax({
            ...
            dataType: 'html',
            success: function (data) {
                $content.eq($this.index()).find('.wi-loader').remove();
