...
if ( ! function_exists( 'wpcom_setup' ) ) :
    function wpcom_setup() {
        ...
        if( defined('WPMX_VERSION') ) {
            global $wpmx_options;
            if( (!isset($wpmx_options['member_group_on']) || (isset($wpmx_options['member_group_on']) && $wpmx_options['member_group_on']=='1')) && file_exists(FRAMEWORK_PATH . '/includes/user-groups.php') ) {
                require_once FRAMEWORK_PATH . '/includes/user-groups.php';
                new WPCOM_User_Groups();
            }

            if( isset($wpmx_options['member_follow']) && $wpmx_options['member_follow']=='1' && file_exists(FRAMEWORK_PATH . '/includes/follow.php') ) {
                require_once FRAMEWORK_PATH . '/includes/follow.php';
                new WPCOM_Follow();
            }

            if( isset($wpmx_options['member_messages']) && $wpmx_options['member_messages']=='1' && file_exists(FRAMEWORK_PATH . '/includes/messages.php') ) {
                require_once FRAMEWORK_PATH . '/includes/messages.php';
                new WPCOM_Messages();
            }

            if( isset($wpmx_options['member_notify']) && $wpmx_options['member_notify']=='1' && file_exists(FRAMEWORK_PATH . '/includes/notifications.php') ) {
                require_once FRAMEWORK_PATH . '/includes/notifications.php';
                $GLOBALS['_notification'] = new WPCOM_Notifications();
            }

            if( isset($wpmx_options['user_card']) && $wpmx_options['user_card']=='1' && file_exists(FRAMEWORK_PATH . '/includes/user-card.php') ) {
...
function wpcom_register_scripts(){
    ...
    if($action==='wp_enqueue_scripts' && !defined('IFRAME_REQUEST') && !wp_script_is('jquery')){
        wp_register_script('jquery-core', includes_url('js/jquery/jquery.min.js'), array(), THEME_VERSION);
    }
    ...
    wp_register_script('main', get_template_directory_uri() . '/js/main.js', array( 'jquery' ), THEME_VERSION, true);
    if(isset($options['iconfont']) && $options['iconfont']) wp_register_script('iconfont', $options['iconfont'], array(), THEME_VERSION, true);
    ...
    <script type="application/ld+json">
        {
            ...
            <?php if($image && $image!== '[]'){ ?>"image": <?php echo wpcom_jsonLd_imgs();?>,<?php } ?>
            "description": "<?php echo utf8_excerpt(strip_tags(get_the_excerpt()), 120);?>",
            "datePublished": "<?php the_time('Y-m-d\TH:i:s');?>",
            "dateModified": "<?php the_modified_time('Y-m-d\TH:i:s');?>",
...
function wpcom_post_submit_cron_fun( $post_ID, $post ) {
    ...
    if($zz_url){
        ...
        wpcom_add_log($post_url . ' - ' . wp_json_encode($post1, JSON_UNESCAPED_UNICODE));
    }
    if($ks_url){
        ...
        wpcom_add_log($post_url . ' - ' . wp_json_encode($post2, JSON_UNESCAPED_UNICODE));
    }
}
...
if ( ! function_exists( 'wpcom_sticky_posts_query' ) && !class_exists('SCPO_Engine') ) :
    ...
    function wpcom_sticky_posts_query( $q ) {
        if( $q->get('post_type') != 'post' && $q->get('post_type') ) return $q;
