...
class WPCOM_DEMO_Importer{
    ...
    public function __construct(){
        if(current_user_can( 'edit_theme_options' )){
            ...
            if(!empty($this->config)){
                $this->config = json_decode(wp_json_encode($this->config),true);
                ...
                add_filter( 'wxr_importer.pre_process.post_meta', array($this, 'page_module_meta') );
                add_filter( 'wp_import_post_meta', array($this, 'page_modules_meta') );
    ...
    public function after_import( $selected_import ) {
        ...
        if($options && isset($options['options'])) {
            ...
            if(version_compare(PHP_VERSION,'5.4.0','<')){
                $o = wp_json_encode($izt_theme_options);
            }else{
                $o = wp_json_encode($izt_theme_options, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
            }
            update_option('izt_theme_options', $o);
        }
        ...
        if($_page_modules) {
            foreach ($_page_modules as $modules) {
                ...
                if(is_string($meta_value)) $meta_value = json_decode($meta_value, true);
                if($meta_value){
                ...
                if(version_compare(PHP_VERSION,'5.4.0','<')){
                    ...
                    $meta_value = wp_json_encode($meta_value, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
                }
        ...
        // replace images url in post metas
        $table2 = $wpdb->prefix.'postmeta';
        $_wpcom_metas2 = $wpdb->get_results("SELECT * FROM `$table2` WHERE meta_key = '_wpcom_metas'");
        if ($_wpcom_metas2) {
            foreach ($_wpcom_metas2 as $metas) {
                $meta_value = maybe_unserialize($metas->meta_value);
                $meta_value = $this->replace_options_images( $meta_value );

                $where = array( 'meta_id' => $metas->meta_id, 'meta_key' => '_wpcom_metas' );
                $wpdb->update( $table2, array('meta_value' => maybe_serialize($meta_value)), $where );
            }
        }
    }
    ...
    function page_module_meta($meta_item){
        if(isset($meta_item['key']) && $meta_item['key'] === '_page_modules'){
            if( $meta_item['value'] && is_string(maybe_unserialize( $meta_item['value'] )) ){
                $meta_item['value'] = wp_slash($meta_item['value']);
            }
        }
        return $meta_item;
    }

    function page_modules_meta($postmeta){
        $_postmeta = array();
        if ( ! empty( $postmeta ) ) {
            foreach ( $postmeta as $meta ) {
                if( $meta['value'] && is_string(maybe_unserialize( $meta['value'] )) ){
    ...
    private function get_file_by_url( $url ){
        global $wpdb;
        $file = array();
        preg_match( '/\/([0-9]{4}\/[0-9]{2}\/[^\/\s]+\.(jpg|png|gif|jpeg|webp|svg))/i', $url, $matches );
