...
class WPCOM_Module_news extends WPCOM_Module {
    function __construct() {
        ...
        parent::__construct( 'news', '新闻列表', $options, 'format_list_bulleted', '/module/mod-news.png' );
    }
    function style( $atts ){
        return array(
            'color' => array(
                ',.sec-title' => '{{color}};',
                '.sec-title-wrap:after,.sec-title-wrap:before, .news-item-month:before' => '{{background-color}};'
            ),
            'tab-color' => array(
                '.module-tab-item' => '{{color}};'
            ),
            'hover-color' => array(
                '.news-item-title:hover,.module-tab-item.active,.module-tab-left .module-tab-item:hover' => '{{color}};',
                ...
                foreach ( $news as $post ) : setup_postdata( $post ); ?>
                    <li class="news-list-item col-sm-12 col-xs-24">
                        <div class="news-item-time">
                            <span class="news-item-day"><?php the_time('d');?></span>
                            <span class="news-item-month"><?php the_time('Y-m')?></span>
                        </div>
                        <div class="news-item-text">
                            <a class="news-item-title" href="<?php echo esc_url( get_permalink() )?>" title="<?php echo esc_attr(get_the_title());?>"<?php echo wpcom_post_target();?>><?php the_title();?></a>
...
register_module( 'WPCOM_Module_news' );